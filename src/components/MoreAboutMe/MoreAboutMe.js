...
const MoreAboutMe = ({ toggleReadMore, toggleReadMoreHandler, isEnglish }) => {
  ...
  return (
    <>
      ...
      <ReadMoreWrapper className={toggleReadMore && 'show'} ref={ref}>
        <ReadMore>
          <Header>{!isEnglish ? 'Who am I exacly?' : 'Kim dokładnie jestem?'}</Header>
          <Answer>
          ...
          <Header>{!isEnglish ? 'How do I learned those things?' : 'Jak się wszystkiego nauczyłem?'}</Header>
          <Answer>
          ...
          <Header>{!isEnglish ? 'Am I ambitios?' : 'Czy jestem ambitny?'}</Header>
          <Answer>
          ...
          <Header>{!isEnglish ? 'Why shoud you hire me?' : 'Czemu powinniście mnie zatrudnić?'}</Header>
          <Answer>
